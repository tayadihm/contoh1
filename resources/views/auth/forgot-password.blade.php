    ...
    <x-auth-card>
        ...
        <form method="POST" action="{{ route('password.email') }}">
            ...
            <div class="flex items-center justify-end mt-4">
                <a href="{{ route('login') }}"
                    class="bg-gray-500 text-xs uppercase my-3 py-3 px-4 shadow-lg rounded-md font-bold text-white focus:outline-none focus:shadow-outline transform transition hover:bg-gray-500 hover:scale-105 duration-300 ease-in-out">{{ __('Kembali') }}</a>
                <x-button
